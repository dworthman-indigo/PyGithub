...
class Organization(github.GithubObject.CompletableGithubObject):
    ...
    def create_dependabot_secret(
        ...
        public_key = self.get_dependabot_public_key()
        ...
        status, headers, data = self._requester.requestJson(
            "PUT", f"{self.url}/dependabot/secrets/{secret_name}", input=put_parameters
        )
        return status == 201
    ...
    def delete_dependabot_secret(self, secret_name):
        """
        :calls: `DELETE /orgs/{org}/dependabot/secrets/{secret_name} <https://docs.github.com/en/rest/dependabot/secrets#delete-an-organization-secret>`_
        :param secret_name: string
        :rtype: bool
        """
        assert isinstance(secret_name, str), secret_name
        status, headers, data = self._requester.requestJson(
            "DELETE", f"{self.url}/dependabot/secrets/{secret_name}"
        )
        return status == 204
    ...
    def get_dependabot_public_key(self):
        """
        :calls: `GET /orgs/{org}/dependabot/secrets/public-key <https://docs.github.com/en/rest/reference/actions#get-an-organization-public-key>`_
        :rtype: :class:`github.PublicKey.PublicKey`
        """
        headers, data = self._requester.requestJsonAndCheck(
            "GET", f"{self.url}/dependabot/secrets/public-key"
        )
